...
void GLWidget::initializeGL()
{
    ...
    QString filesPath = QCoreApplication::applicationDirPath() + "/meshes/";
    ...
    vertexShaderFile = "/shaders/olento.vertexshader";
    fragmentShaderFile = "/shaders/olento.fragmentshader";

    cerr << "   Käytetään shadereita: \n";
    cerr << "   " << vertexShaderFile.toStdString() << ",\n";
    cerr << "   " << fragmentShaderFile.toStdString() << "\n";

    if(!m_program->addShaderFromSourceFile(QOpenGLShader::Vertex, QCoreApplication::applicationDirPath() + vertexShaderFile)) {
        //cerr << "Couldn't add vertex shader: " << vertexShaderFile.toStdString() << "\n";
        cerr << "InitializeGL failed!\n";
        return;
    }

    if(!m_program->addShaderFromSourceFile(QOpenGLShader::Fragment, QCoreApplication::applicationDirPath() + fragmentShaderFile)) {
